...
// Parse any CSS color string (#hex, rgb()/rgba(), hsl()/hsla(), bare "H, S%, L%")
// into [r,g,b]. Returns null if it can't be resolved.
function colorToRgb(input) {
  let color = String(input || '').trim();
  ...
  let hslMatch = color.match(/^hsla?\(\s*([\d.]+)[,\s]+([\d.]+)%[,\s]+([\d.]+)%/i);
  if (hslMatch) {
    return hslToRgb(+hslMatch[1], +hslMatch[2] / 100, +hslMatch[3] / 100);
  }
  let bareMatch = color.match(/^([\d.]+)\s*,\s*([\d.]+)%\s*,\s*([\d.]+)%$/);
  if (bareMatch) {
    return hslToRgb(+bareMatch[1], +bareMatch[2] / 100, +bareMatch[3] / 100);
  }
  return null;
}

// Turn any color string into an "H, S%, L%" triplet for use inside hsla(var(--x))
function toHslTriplet(input, fallback) {
  let color = String(input || '').trim();
  let hslMatch = color.match(/^hsla?\(\s*([\d.]+)[,\s]+([\d.]+)%[,\s]+([\d.]+)%/i);
  if (hslMatch) {
    return `${Math.round(+hslMatch[1])}, ${Math.round(+hslMatch[2])}%, ${Math.round(+hslMatch[3])}%`;
  }
  let bareMatch = color.match(/^([\d.]+)\s*,\s*([\d.]+)%\s*,\s*([\d.]+)%$/);
  if (bareMatch) {
    return `${Math.round(+bareMatch[1])}, ${Math.round(+bareMatch[2])}%, ${Math.round(+bareMatch[3])}%`;
  }
  let rgb = colorToRgb(color);
  if (!rgb) return fallback;
  return rgbToHsl(rgb[0], rgb[1], rgb[2]).join(', ');
}

// SCSS-style mix(colorA, colorB, weightA) on RGB, weightA as a 0-1 fraction
function mixRgb(a, b, weightA) {
  return [0, 1, 2].map(i => Math.round(a[i] * weightA + b[i] * (1 - weightA)));
}

// --- Accent -----------------------------------------------------------------
let accentInput = params.accentColor === 'custom' ? params.customAccentColor : params.accentColor;
let accentHSL = toHslTriplet(accentInput, '45, 80%, 65%');
let accentRgb = colorToRgb(accentInput);

// --- Background + text ------------------------------------------------------
const TERMINAL_BASE = [29, 30, 40]; // #1D1E28 — original Terminal background base
let backgroundHSL;
let colorHSL;
...
if (params.colorScheme === 'light') {
  ...
  colorHSL = '30, 22%, 9%';
} else if (params.colorScheme === 'custom') {
  backgroundHSL = toHslTriplet(params.backgroundColor, '248, 11%, 15%');
  colorHSL = toHslTriplet(params.textColor, '0, 0%, 100%');
} else {
  // dark (default): background = mix(accent, #1D1E28, 2%) — same as Hugo Terminal
  let bgRgb = accentRgb ? mixRgb(accentRgb, TERMINAL_BASE, 0.02) : TERMINAL_BASE;
  backgroundHSL = rgbToHsl(bgRgb[0], bgRgb[1], bgRgb[2]).join(', ');
  colorHSL = '0, 0%, 100%';
}
